...
namespace UnityEngine.Rendering.Universal
{
    class IBLFilterGGX : IBLFilterBSDF
    {
        ...
        public override void Initialize(UniversalRenderPipelineRuntimeShaders runtimeShaders)
        {
            ...
            //{
            //    m_PlanarReflectionFilteringCS = m_RuntimeShaders.planarReflectionFilteringCS;
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.FilterCubemapGGX)))
            {
                ...
                for (int mip = 1; mip < (int)EnvConstants.ConvolutionMipCount; ++mip)
                {
                    ...
                    for (int face = 0; face < 6; ++face)
                    {
                        ...
                        var transform = RenderingUtils.ComputePixelCoordToWorldSpaceViewDirectionMatrix(0.5f * Mathf.PI, Vector2.zero, faceSize, worldToViewMatrices[face], true);
