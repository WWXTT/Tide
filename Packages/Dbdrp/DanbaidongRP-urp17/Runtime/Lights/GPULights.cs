...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class GPULights : ScriptableRenderPass
    {
        ...
        private void InitResources(RenderGraph renderGraph, GPULightsPassData passData, UniversalLightData lightData, GPULightsOutPassData outData, UniversalCameraData cameraData)
        {
            ...
            passData.AABBBoundsBuffer = renderGraph.CreateBuffer(new BufferDesc(allLightsBufferSize, Marshal.SizeOf(typeof(float4))));
            passData.coarseLightList = renderGraph.CreateBuffer(new BufferDesc(LightDefinitions.s_MaxNrBigTileLightsPlusOne * passData.nrBigTilesX * passData.nrBigTilesY, sizeof(uint)));
            ...
            passData.globalLightListAtomic = renderGraph.CreateBuffer(new BufferDesc(1, sizeof(uint)));
            passData.perVoxelLightLists = renderGraph.CreateBuffer(new BufferDesc(32 * (1 << k_Log2NumClusters) * nrClusterTiles, sizeof(uint))); ;
            passData.perVoxelOffset = renderGraph.CreateBuffer(new BufferDesc((int)LightCategory.Count * (1 << k_Log2NumClusters) * nrClusterTiles, sizeof(uint)));
            passData.perTileLogBaseTweak = renderGraph.CreateBuffer(new BufferDesc(nrClusterTiles, sizeof(float)));

            // Outdata
            outData.lightListCB = passData.lightListCB;
            outData.GPULightsData = renderGraph.CreateBuffer(new BufferDesc(m_MaxPunctualLightsOnScreen, Marshal.SizeOf(typeof(GPULightData))));
            outData.directionalLightsData = renderGraph.CreateBuffer(new BufferDesc(m_MaxDirectionalLightsOnScreen, Marshal.SizeOf(typeof(DirectionalLightData))));
