...
namespace UnityEngine.Rendering.Universal
{
    ...
    internal class RayTracingSystem : CameraRelatedSystem<RayTracingSystem>
    {
        ...
        private static bool UpdateMaterialCRC(EntityId matEntityId, int matCRC)
        {
            int matPrevCRC;
            if (m_MaterialCRCs.TryGetValue(matEntityId, out matPrevCRC))
            {
                m_MaterialCRCs[matEntityId] = matCRC;
                return (matCRC != matPrevCRC);
            }
            else
            {
                m_MaterialCRCs.Add(matEntityId, matCRC);
                return true;
            }
        }
...
#endif
            {
                ...
                if (effectParameters.pathTracing)
                {
                    ...
                    for (int i = 0; i < cullingResults.materialsCRC.Length; i++)
                    {
                        ...
                        m_AccelerationStructureSystem.materialsDirty |= UpdateMaterialCRC(matCRC.entityId, matCRC.crc);
                    }
                }
