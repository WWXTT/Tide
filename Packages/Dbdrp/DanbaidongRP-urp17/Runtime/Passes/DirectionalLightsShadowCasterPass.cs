    ...
    public class DirectionalLightsShadowCasterPass : ScriptableRenderPass
    {
        ...
        internal TextureHandle Render(RenderGraph graph, ContextContainer frameData)
        {
            ...
            using (var builder = graph.AddUnsafePass<PassData>("Directional Lights Shadowmap", out var passData, base.profilingSampler))
            {
                ...
                {
                    // Shaders declare _DirectionalLightsShadowmapTexture as a 2DArray, but the RG default
                    // shadow texture is 2D (core 6000.5 removed defaultShadowArrayTexture).
                    // Allocate an empty 1x1 2DArray instead to avoid a texture dimension mismatch.
                    var emptyDescriptor = ShadowUtils.GetTemporaryShadowTextureDescriptor(1, 1, 16);
                    emptyDescriptor.dimension = TextureDimension.Tex2DArray;
                    emptyDescriptor.volumeDepth = 1;

                    shadowTexture = UniversalRenderer.CreateRenderGraphTexture(graph, emptyDescriptor, "_DirectionalLightsShadowmapTexture", true, ShadowUtils.m_ForceShadowPointSampling ? FilterMode.Point : FilterMode.Bilinear);
                }
