Shader "DanbaidongRP/PBRToon/Stockings"
{
    Properties
    {
        ...
        [FoldoutBegin(_FoldoutStockingsEnd)]_FoldoutPBRProp("Stockings Properties", Float) = 0
            _StockingsPow                           ("StockingsPow", Range(1, 5))           = 1.5
            _StockingsColorInside                   ("ColorInside", Color)                  = (1,1,1,1)
            _StockingsColorOutside                  ("ColorOutside", Color)                 = (0.5,0.5,0.5,1)
        ...
        [FoldoutBegin(_FoldoutDirectLightEnd)]_FoldoutDirectLight("Direct Light", Float) = 0
            ...
            _ShadowSmoothNdotL                      ("ShadowSmoothNdotL", Range(0, 1))      = 0.1
        ...
        [FoldoutBegin(_FoldoutEmissRimEnd)]_FoldoutEmissRim("Emission, Rim, etc.", Float) = 0
            ...
            // Stockings rim default is zero
            _DirectRimWidth                         ("DirectRimWidth", Range(0, 10))        = 0
            _PunctualRimWidth                       ("PunctualRimWidth", Range(0, 10))      = 0
        ...
        [FoldoutBegin(_FoldoutOutlineEnd, PassSwitch, CharacterOutline)]_FoldoutOutline("Outline", Float) = 0
            ...
            _OutlineClampScale                      ("ClampScale", Range(0.01, 5))          = 1.0
            // Stockings outline default is no lighting
            [Title(Lighting)]
            [HDR]_OutlineDirectLightingColor        ("DirectColor", Color)                  = (1,1,1,0.5)
            _OutlineDirectLightingOffset            ("DirectOffset", Range(-1, 1))          = -1.0
            [HDR]_OutlinePunctualLightingColor      ("PunctualColor", Color)                = (1,1,1,0.5)
            _OutlinePunctualLightingOffset          ("PunctualOffset", Range(-1, 1))        = -1.0
            ...
            float4 ForwardToonFrag(Varyings i) : SV_Target0
            {
                ...
                for (dirLightIndex = 0; dirLightIndex < _DirectionalLightCount; dirLightIndex++)
                {
                    ...
                    #endif
                    {
                        ...
                        // We don't use Fresnel term for Stockings
                        float3 F = fresnel0;
                        float D = D_GGX(NdotH, clampedRoughness);
                        float Vis = V_SmithJointGGX(abs(NdotL), clampedNdotV, clampedRoughness);
                        float3 specTerm = F * D * Vis;
                        float diffTerm = Lambert();

                        #ifdef _SHADOW_RAMP
                        // Remap D range and strength. Use roughnessSquare as a mask.
                        float specRange = D * roughnessSquare * PI;
                        float3 specRampCol = SampleDirectSpecularRamp(TEXTURE2D_ARGS(_ShadowRampTex, sampler_ShadowRampTex), specRange).xyz;
                        specTerm = F * clamp(specRampCol.rgb, 0, 10) * saturate(Vis) / roughnessSquare * 0.7;
                        #endif
                        ...
                        // Accumulate
                        directLighting.diffuse += diffuseColor * shadowRamp * dirLight.lightColor * directOcclusion;
                        directLighting.specular += shadowRamp * specTerm * dirLight.lightColor * directOcclusion; // Stockings specular also use base shadowRamp as scattering color.
                        rimColor += directRim;
                    }
                }
