...
namespace MCPForUnity.Editor.Clients
{
    ...
    public abstract class JsonFileMcpConfigurator : McpClientConfiguratorBase
    {
...

                {
                    ...
                    if (rootConfig != null)
                    {
                        ...
                        unityToken = client.IsVsCodeLayout
                            ...
                            : GetNestedContainer(rootConfig, containerKey)?["unityMCP"];
                    }
        ...
        /// <summary>
        /// Resolves a (possibly dotted) container key path against a config root,
        /// e.g. "mcpServers" or ZCode's "mcp.servers". Returns null when any
        /// segment is missing or not an object.
        /// </summary>
        internal static JObject GetNestedContainer(JObject root, string dottedKey)
        {
            JToken current = root;
            foreach (string segment in dottedKey.Split('.'))
            {
                if (current is JObject obj && obj[segment] is JObject next)
                {
                    current = next;
                }
                else
        ...
        public override void Unregister()
        {
            ...
            try
            {
                ...
                else
                {
                    string containerKey = string.IsNullOrEmpty(client.ServerContainerKey)
                        ? "mcpServers" : client.ServerContainerKey;
                    if (GetNestedContainer(root, containerKey)?.Remove("unityMCP") == true) removed = true;
                }
