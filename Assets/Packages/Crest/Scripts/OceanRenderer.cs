...
#error This version of Crest requires Unity 2022.3 or later.
#endif
...
#pragma warning restore 414
        ...
        void OnEnable()
        {
            ...
            if (RenderPipelineHelper.IsHighDefinition && UnderwaterRenderer.Instance == null)
            {
...
#pragma warning restore 0618
        ...
        void LateUpdatePrimaryLight()
        {
#if UNITY_2022_3_OR_NEWER
#if UNITY_EDITOR
            // Solves NullReferenceException from hdCamera.volumeStack further below.
            if (!Application.isPlaying && _isFirstFrameSinceEnabled)
            {
                return;
            }
#endif
            ...
            var sun = _primaryLight;
            var lightDirection = Vector3.zero;
            var lightIntensity = Color.clear;

            if (sun != null && sun.enabled && sun.TryGetComponent<HDAdditionalLightData>(out var data))
            {
#if UNITY_2022_3_OR_NEWER
...
#else
                lightDirection = -sun.transform.forward;
                lightIntensity = data.color * data.intensity;
    ...
    public partial class OceanRenderer : IValidated
    {
        ...
        public bool Validate(OceanRenderer ocean, ValidatedHelper.ShowMessage showMessage)
        {
            ...
            if (RenderPipelineHelper.IsHighDefinition)
            {
                ...
                var camera = ocean._camera != null ? ocean._camera : Camera.main;
                ...
                if (Application.isPlaying && showMessage == ValidatedHelper.HelpBox)
                {
                    if (hdCamera?.frameSettings.IsEnabled(FrameSettingsField.Refraction) == false && material != null && material.IsKeywordEnabled("_SURFACE_TYPE_TRANSPARENT"))
                    {
                        showMessage
                        (
                            "Refraction is disabled. Transparency requires it to work.",
                            "Enable it in the frame settings on the camera, or the default frame settings in the global settings.",
                            ValidatedHelper.MessageType.Error, hdCamera.camera
                        );
                    }
                }
            }
            ...
            foreach (var asset in GraphicsSettings.allConfiguredRenderPipelines)
            {
                if (asset is UniversalRenderPipelineAsset urpAsset)
                {
                    if (urpAsset.opaqueDownsampling != Downsampling.None)
                    {
                        showMessage
                        (
                            $"A {nameof(UniversalRenderPipelineAsset)} has Opaque Downsampling enabled. This can cause an outline to appear around objects underwater.",
                            "Disable Opaque Downsampling.",
                            ValidatedHelper.MessageType.Warning, urpAsset
                        );
                    }
                }
            }
