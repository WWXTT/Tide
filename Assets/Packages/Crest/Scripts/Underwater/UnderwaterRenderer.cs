    ...
    public partial class UnderwaterRenderer : IValidated
    {
        public bool Validate(OceanRenderer ocean, ValidatedHelper.ShowMessage showMessage)
        {
            ...
            if (RenderPipelineHelper.IsHighDefinition)
            {
                ...
                if (!hdAsset.currentPlatformRenderPipelineSettings.supportCustomPass)
                {
                    ...
                    (
                        ...
                        "Enabled them on the HDRP global asset.",
                        ValidatedHelper.MessageType.Error, camera
                    );
                }
                ...
                if (Application.isPlaying && showMessage == ValidatedHelper.HelpBox)
                {
                    if (!hdCamera.frameSettings.IsEnabled(FrameSettingsField.CustomPass))
                    {
                        ...
                        (
                            ...
                            "Enable them in the frame settings on the camera or the default frame settings in the HDRP global settings.",
                            ValidatedHelper.MessageType.Error, camera
                        );
                    }
                }
            ...
            if (ocean != null && ocean.OceanMaterial != null)
            {
                ...
                if (material.shader.name != "Crest/Ocean" && !material.IsKeywordEnabled("_UNDERWATER_ON"))
                {
                    showMessage
                    (
                ...
                if (material.shader.name != "Crest/Ocean" && material.GetFloat("_CullMode") == (int)CullMode.Back)
                {
                    showMessage
                    (
