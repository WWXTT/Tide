...
namespace Crest
{
    ...
    partial class UnderwaterEffectPassURP : ScriptableRenderPass
    {
        ...
        const string SHADER_UNDERWATER_EFFECT_SG = "Hidden/Crest/Underwater/Underwater Effect SG";
        ...
        {
            if (_underwaterEffectMaterial != null)
            {
        ...
        static void EnqueuePass(ScriptableRenderContext context, Camera camera)
        {
            ...
            if (!Helpers.MaskIncludesLayer(camera.cullingMask, ur._layer))
            {
                return;
            }
...
#endif
        {
            ...
            if (_firstRender || _isShaderGraph)
            {
                if (_firstRender || oceanMaterial.shader.name == "Crest/Ocean URP")
                {
                    if (_underwaterEffectMaterial != null) CoreUtils.Destroy(_underwaterEffectMaterial.material);
                    ...
                    _firstRender = true;
                    _isShaderGraph = false;
                }
            }
            else
            {
                if (_firstRender || oceanMaterial.shader.name == "Crest/Ocean")
                {
                    if (_underwaterEffectMaterial != null) CoreUtils.Destroy(_underwaterEffectMaterial.material);
                    _underwaterEffectMaterial = new PropertyWrapperMaterial(SHADER_UNDERWATER_EFFECT_SG);
                    _underwaterEffectMaterial.material.hideFlags = HideFlags.HideAndDontSave;
                    _firstRender = true;
                    _isShaderGraph = true;
                }
...
#if UNITY_6000_0_OR_NEWER
            // The last pass for RG (uber post) does not resolve the texture.
            // This is wasteful if the pass after this does a resolve.
            // Possibly a bug with Unity or RG specific?
