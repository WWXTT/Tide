...
// #endif
		...
		const float3 lightDir = _PrimaryLightDirection;
		const float3 lightCol = _PrimaryLightIntensity;
#else
	...
	return lerp(sceneColour, volumeLight, saturate(1.0 - exp(-_DepthFogDensity.xyz * fogDistance)));
}
...
float4 Frag(Varyings input) : SV_Target
{
	...
	float rawDepth = LoadSceneDepth(input.positionCS.xy);
