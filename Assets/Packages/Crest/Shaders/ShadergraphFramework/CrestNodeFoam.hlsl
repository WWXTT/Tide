...
)
{
	half ft = lerp
	(
		i_texture.Sample((1.25 * (i_worldXZ0 + i_texelOffset) + _CrestTime / 10.0) / (i_texelSize0 * i_texture._scale / 25.0)).r,
		i_texture.Sample((1.25 * (i_worldXZ1 + i_texelOffset) + _CrestTime / 10.0) / (i_texelSize1 * i_texture._scale / 25.0)).r,
		i_lodVal
	);
...
)
{
	...
	worldXZ0 -= foamTexture.FloatingOriginOffset(_OceanPosScale0);
	worldXZ1 -= foamTexture.FloatingOriginOffset(_OceanPosScale1);
	...
	half whiteFoam = WhiteFoamTexture(foamTexture, i_foam, i_feather, worldXZ0, worldXZ1, (float2)0.0, _OceanPosScale0.z, _OceanPosScale1.z, i_lodVal);
	...
	half whiteFoam_x = WhiteFoamTexture(foamTexture, i_foam, i_feather, worldXZ0, worldXZ1, dd.xy, _OceanPosScale0.z, _OceanPosScale1.z, i_lodVal);
	half whiteFoam_z = WhiteFoamTexture(foamTexture, i_foam, i_feather, worldXZ0, worldXZ1, dd.yx, _OceanPosScale0.z, _OceanPosScale1.z, i_lodVal);
