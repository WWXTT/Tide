...
)
{
	...
	half ft = lerp
	(
		i_texture.SampleLevel((foamUVBubbles0 + parallaxOffset) / (i_texelSize0 / 25.0), 3.0).r,
		i_texture.SampleLevel((foamUVBubbles1 + parallaxOffset) / (i_texelSize1 / 25.0), 3.0).r,
		i_lodVal
	);
...
)
{
	...
	worldXZ.displaced0   -= foamTexture.FloatingOriginOffset(_OceanPosScale0);
	worldXZ.displaced1   -= foamTexture.FloatingOriginOffset(_OceanPosScale1);
	worldXZ.undisplaced0 -= foamTexture.FloatingOriginOffset(_OceanPosScale0);
	worldXZ.undisplaced1 -= foamTexture.FloatingOriginOffset(_OceanPosScale1);
