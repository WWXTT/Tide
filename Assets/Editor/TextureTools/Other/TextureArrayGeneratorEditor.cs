...
public class TextureArrayGeneratorEditor : EditorWindow
{
    ...
    // 菜单入口集中在 FTextureEditWindows_MenuItems（Tools/美术/创建贴图数组）
    ...
    private void ConfigureImportSettings(string assetPath, TextureKind kind)
    {
        ...
        // 法线图同理（sRGB=0）：法线是方向数据，Gamma 采样会让中性灰 0.5
        // 读成 ~0.21，解包出的法线整体偏向 (-x,-y)，凹凸细节被压扁。
        importer.sRGBTexture = settings.sRGB;

        // 源贴图是 DX 风格（文件名 _Normal_DX），绿通道与 Unity 的 OpenGL 约定相反，
        // 不翻转的话凹凸方向是反的（坑当包、包当坑）
    ...
    private void CreateTexture2DArrayAsset(List<Texture2D> slices, string assetPath)
    {
        ...
        // 地形以掠射角观察为主，各向异性过滤直接决定远/侧向的清晰度（1 = 关闭，
        // 远处会被 mip 糊成一片）
